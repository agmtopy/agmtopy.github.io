...
__AbstractBeanFactory__主要有三个实现类__XmlBeanFactory(已过时)__、__AbstractAutowireCapableBeanFactory__、__DefaultListableBeanFactory__
...
可以看到lombok的@Builder注解只是在内部设置了一个静态方法和静态类来生成build方式，将__Direct__的职责交给类的使用者去处理了。
